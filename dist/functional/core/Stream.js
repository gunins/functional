(function (global, factory) {
	...
	(factory((global['functional/core/Stream'] = global['functional/core/Stream'] || {}, global['functional/core/Stream'].js = {}),global.Maybe_mjs,global.Task_mjs,global.option_mjs,global.storage_mjs));
}(this, (function (exports,Maybe_mjs,Task_mjs,option_mjs,storage_mjs) { 'use strict';
...
const toFunction = (job) => option_mjs.option()
    .or(isFunction(job), () => Maybe_mjs.some(job))
    .or(isDefined(job), () => Maybe_mjs.some(() => job))
    .finally(() => Maybe_mjs.some(_ => _));

const emptyFn = _ => _;


const getRoot = (instance, onReady) => option_mjs.option()
...
const setContextStorage = (context, contextID) => context
    .get(contextID)
    .getOrElseLazy(() => context.set(contextID, storage_mjs.storage())
        .get());
...
class Stream {
    ...
    constructor(job, parent) {
        ...
        this[_refs] = storage_mjs.storage();
        this[_stream] = storage_mjs.storage();
        this[_contextStorage] = storage_mjs.storage();
        this[_onStreamFinishHandlers] = storage_mjs.storage();
        this[_onStreamErrorHandlers] = storage_mjs.storage();
   ...
   /* // return new stream instance
    ...
    // Will take a task
    throughTask(_task) {
        return this.map(_ => Task_mjs.task(_).through(_task).unsafeRun())
    };
